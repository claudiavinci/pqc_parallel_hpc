...
def compute_baseline(df: DataFrame):
    baselines = {}
    baseline = df[df["OMP_ENABLED"] == 0]["TIME_SEC"].mean().round(3)
    baselines["TOTAL"] = baseline
    for name, metric in STAGES_METRICS.items():
        baseline = df[df["OMP_ENABLED"] == 0][metric].mean().round(3)
        baselines[name] = baseline
    return baselines

def compute_baseline_metrics(baselines: dict):
    # Calcola il rapporto tra il tempo del collo di bottiglia e lo stadio più veloce
    service_times = {
        name: (baselines[name] / N_JOBS) * 1000 for name in STAGES_METRICS.keys()
    }

    max_stage_time = max(service_times.values())
    min_stage_time = min(service_times.values())
    ...
    throughput_per_stage = {
        name: (N_JOBS / baselines[name]).round(3) for name in STAGES_METRICS.keys()
    }
...
def preprocess_dataframe(df:DataFrame, groupby_cols:list, stages=False):
    if stages:
...
def add_speedup_efficiency(df: DataFrame, baseline, metric="TIME_SEC", prefix=""):
    speedup = f"{prefix}SPEEDUP"
    efficiency = f"{prefix}EFFICIENCY"

    df[speedup] = (baseline / df[metric]).round(3)

    df[efficiency] = (df[speedup] / df["TOT_WORKERS"]).round(3)

def extract_results(row, kind, metric="TIME_SEC", prefix=""):
    speedup = f"{prefix}SPEEDUP"
    efficiency = f"{prefix}EFFICIENCY"
...
def evaluate_metrics(df: DataFrame, baselines, df_name, stages=False):
    results = {}

    # ----------METRICHE GLOBALI --------------------

    add_speedup_efficiency(df, baselines["TOTAL"])
    ...
    if stages:
        ...
        for name, metric in STAGES_METRICS.items():
            ...
            add_speedup_efficiency(df, baselines[name], metric=metric, prefix=prefix)

            speedup_col = f"{prefix}SPEEDUP"
            efficiency_col = f"{prefix}EFFICIENCY"

            best_speedup = df.loc[df[speedup_col].idxmax()]

            best_efficiency = df.loc[df[efficiency_col].idxmax()]

            results["stages"][name.lower()] = {
                "speedup": extract_results(best_speedup,"speedup",metric=metric,prefix=prefix),
                "efficiency": extract_results(best_efficiency,"efficiency",metric=metric,prefix=prefix),
            }
...
if __name__ == "__main__":
    ...
    compute_baseline_metrics(baselines)
    ...
    seq_omp_mean = preprocess_dataframe(seq_omp_df, ["TOT_WORKERS", "OMP_THREADS"], stages=True)
    ...
    best_res = {
        "seq_omp": evaluate_metrics(seq_omp_mean, baselines, "seq_omp", stages=True),
    ...
    for name in STAGES_METRICS.keys():
        plot_metrics(seq_omp_mean,
                    colx="TOT_WORKERS",
